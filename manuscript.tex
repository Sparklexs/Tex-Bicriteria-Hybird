...
\begin{theorem}
	$ \pi $ can be computed in $ O(\log^2 n) $ time and $ O(n) $ time with its time cost $ t(\pi) \leqslant f(\pi^\star) + t_{max} $ and space cost $ s(\pi) \leqslant T + 2s_{max} $.
\end{theorem}
...
\begin{equation}
L^\star = L(\mu^\star) = \max_{\mu \geqslant 0} L(\mu) =\max_{\mu \geqslant 0} \min_{\pi \in \Pi} f(\pi) + \mu g(\pi)
\end{equation}

\noindent Now the problem has been converted from searching for an optimal path to a linear programming calculating maximal $ L(\mu) $, where each path defines a feasible region.
...
and $ L' $ is the maximum value for the current lower envelope, however
